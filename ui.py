...
class BrightosUI:
    ...
    def wait_for_api_key(self):
        """Block until API key is saved (called from main thread before starting BRIGHTOS)."""
    ...
    def _show_setup_ui(self):
        ...
        tk.Label(self.setup_frame,
                 text="Enter your Gemini API key to boot BRIGHTOS.",
    ...
    def _save_api_keys(self):
        ...
        self.status_text = "ONLINE"
        self.write_log("SYS: Systems initialised. BRIGHTOS online.")


class Brightos3DUI:
    """
    3D Avatar UI using pywebview and Convai Web SDK.
    Compatible with the original BrightosUI interface.
    """
    def __init__(self, html_path):
        if webview is None:
            raise ImportError("pywebview is not installed. Please run 'pip install pywebview'")
        ...
        # Internal state to match BrightosUI
        self.speaking = False
        self.status_text = "INITIALISING"
    ...
    def write_log(self, text: str):
        # Forward to JS for logging in the 3D UI
        if self.window:
            try:
                # Check if it's an AI message to trigger the 3D avatar
                tl = text.lower()
                if "brightos:" in tl or "ai:" in tl:
                    clean_text = text.split(":", 1)[1].strip() if ":" in text else text
                    self.window.evaluate_js(f"if(window.py_speak) window.py_speak({json.dumps(clean_text)})")
                
                # Update status based on message type
                status = "PROCESSING" if "you:" in tl else "RESPONDING" if "ai:" in tl else "ONLINE"
                self.window.evaluate_js(f"if(window.py_setStatus) window.py_setStatus({json.dumps(status)})")
            except:
                pass
    ...
    def start_speaking(self):
        ...
        if self.window:
            try:
                self.window.evaluate_js("if(window.py_setStatus) window.py_setStatus('SPEAKING')")
            except:
                pass
    ...
    def stop_speaking(self):
        ...
        if self.window:
            try:
                self.window.evaluate_js("if(window.py_setStatus) window.py_setStatus('ONLINE')")
            except:
                pass

    def run(self):
        """Starts the pywebview window. This is blocking."""
        self.window = webview.create_window(
            "BRIGHTOS - Advanced AI Interface",
