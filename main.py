...
def _update_memory_async(user_text: str, brightos_text: str) -> None:
    """
...
class BrightosLive:
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[BRIGHTOS] 🔧 TOOL: {name}  ARGS: {args}")
        ...
        try:
            if name == "open_app":
                ...
                # NATIVE VISION: Don't start a separate process! Use the current session.
                angle = args.get("angle", "screen").lower()
                text = args.get("text", "What do you see?")
                ...
                try:
                    ...
                    if frame_data:
                        ...
                        result = "Vision frame sent. I am now analyzing it."
                    else:
                        result = "Failed to capture vision frame."
                except Exception as e:
                ...
                result = f"Real-time vision is now {'enabled' if self.vision_active else 'disabled'}."
        ...
        print(f"[BRIGHTOS] 📤 {name} → {result[:80]}")
    ...
    async def _listen_audio(self):
        ...
        try:
            ...
            print(f"[BRIGHTOS] ❌ Mic error: {e}")
            raise
        finally:
            stream.close()

    async def _receive_audio(self):
        print("[BRIGHTOS] 👂 Recv started")
        out_buf = []
        in_buf  = []
        ...
        try:
            while True:
                ...
                async for response in turn:
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if out_buf:
                                ...
                                if full_out:
                                    self.ui.write_log(f"Brightos: {full_out}")
                            out_buf = []
                    ...
                    # Bu kısım 'Warning: non-data parts' hatasını engeller ve metni yakalar
                    if not response.server_content and not response.tool_call:
                        try:
                            # Audio verisini güvenli al
                    ...
                    if response.tool_call:
                        ...
                        for fc in response.tool_call.function_calls:
                            print(f"[BRIGHTOS] 📞 Tool call: {fc.name}")
            ...
            print(f"[BRIGHTOS] ❌ Recv error: {e}")
            traceback.print_exc()
            raise

    async def _play_audio(self):
        print("[BRIGHTOS] 🔊 Play started")
        stream = await asyncio.to_thread(
            pya.open,
            format=FORMAT,
            channels=CHANNELS,
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    print("[BRIGHTOS] ✅ Connected.")
                    self.ui.write_log("BRIGHTOS online.")
                ...
                print(f"[BRIGHTOS] ⚠️  Error: {e}")
                traceback.print_exc()

            print("[BRIGHTOS] 🔄 Reconnecting in 3s...")
            await asyncio.sleep(3)

def main():
    from ui import Brightos3DUI
    
    # Yeni 3D Arayüzü Başlat
    ...
    def runner():
        ...
        brightos = BrightosLive(ui)
        try:
            asyncio.run(brightos.run())
        except KeyboardInterrupt:
            print("\n🔴 Kapatılıyor...")

    # Arka planda BRIGHTOS zekasını çalıştır
    threading.Thread(target=runner, daemon=True).start()
    
    # Ana pencereyi (3D Avatar) başlat
    ui.run()
